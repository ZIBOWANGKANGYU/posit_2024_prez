...
# multiple stocks with a dropdown select input. It should show 
# a table with information about all selected stocks, and a line 
...
![](images/context_files.jpg){width="41"}
...
-   Iteration 3: Refer to resources on the internet 
...
test_that("stock app server function retrieves stock data", {
  # Create a mock input object
  input <- list(stock = c("AAPL", "GOOGL"))
  
  # Test the get_stock_data function
  testServer(server, args = list(input = input), {
    ...
  })
})
```

Correct testing syntax created. 
